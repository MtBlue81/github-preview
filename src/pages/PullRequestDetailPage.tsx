...
export function PullRequestDetailPage() {
  ...
  const { getNextPR, getPreviousPR, currentPRIndex, allPullRequests } = useNavigationStore();
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft' || e.key === 'h') {
        ...
        if (prevPR) {
          navigate(`/pr/${prevPR.repository.owner.login}/${prevPR.repository.name}/${prevPR.number}`);
        }
      } else if (e.key === 'ArrowRight' || e.key === 'l') {
        const nextPR = getNextPR();
        if (nextPR) {
          navigate(`/pr/${nextPR.repository.owner.login}/${nextPR.repository.name}/${nextPR.number}`);
        }
    ...
    <Layout>
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => prevPR && navigate(`/pr/${prevPR.repository.owner.login}/${prevPR.repository.name}/${prevPR.number}`)}
            disabled={!prevPR}
          ...
          <button
            onClick={() => nextPR && navigate(`/pr/${nextPR.repository.owner.login}/${nextPR.repository.name}/${nextPR.number}`)}
            disabled={!nextPR}
            className="px-3 py-1 text-sm border rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            title="次のPR (→ または l)"
          >
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center text-sm text-gray-600">
          <span className={`px-2 py-1 rounded ${pr.state === 'OPEN' ? 'bg-green-100 text-green-800' : pr.state === 'MERGED' ? 'bg-purple-100 text-purple-800' : 'bg-red-100 text-red-800'}`}>
            {pr.state}
          </span>
          <span className="ml-3">
            {pr.author.login} wants to merge {pr.commits.totalCount} commits into {owner}/{repo}
          </span>
        </div>
      </div>
                ...
                <div key={item.id} className="flex space-x-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center">
                      <h4 className="text-sm font-medium text-gray-900">{item.author.login}</h4>
                      ...
                      {'state' in item && (
                        <span className={`ml-2 px-2 py-1 text-xs rounded ${
                          ...
                          item.state === 'CHANGES_REQUESTED' ? 'bg-red-100 text-red-800' :
